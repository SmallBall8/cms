<template>
  <div class="main-menu">
    ...
    <div class="logo">
      ...
      <h2 v-show="!isFold" class="title">鸿源管理系统</h2>
    </div>
    <!--菜单-->
    <div class="menu">
      <el-menu
      ...
      >
        ...
        <template v-for="item in userMenus" :key="item.id">
          <el-sub-menu :index="item.id + ''">
            <template #title>
              <!--字符串：el-icon-monitor   组件，动态组件的使用-->
...
//1.获取动态的菜单,虽然是存在localstorage中，但是我也存在了store中，所以可以从store中拿
const loginStore = useLoginStore()
const userMenus = loginStore.userMenus
console.log('xixixi', userMenus)

//2.监听item的点击
const router = useRouter()
function handleItemClick(item: any) {
  //console.log(item) //会输出的里面有url
  const url = item.url
  router.push(url)
}
</script>
